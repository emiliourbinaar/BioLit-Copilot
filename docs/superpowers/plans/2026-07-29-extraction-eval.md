...
- **That precedent is about LOGIC, not WIRING, and Task 8 showed the difference matters.** All five of `run_extract_eval`'s gate call sites were unpinned: deleting `assert_dataset_size`, `assert_gold_sentence_regression_pin`, `assert_bucket_closure`, `assert_gold_sentence_recall_anchor`, or `assert_gold_relation_ceiling` **from the runner** left the whole suite green, because each gate was tested as a function while nothing proved the runner invokes it. Testing that a call is reachable is not the same as testing what it computes, so the five `test_the_run_*` tests target the runner directly and the untested-`main()` precedent still holds unchanged. The general rule: anything `main()` orchestrates that can be tested belongs in a tested function, and every gate that function is supposed to call needs a test that fails when the CALL is removed.
...
- **Every operand of a compound condition gets its own fixture (ADR-0014).** Each disjunct and each conjunct must be independently exercised; the operational test is that replacing the condition with either operand alone must turn **some gate** red — and that gate is sometimes `pyright`, not `pytest` (Task 6b's `canonical_id is None` disjunct is pinned only by the type checker; record the verbatim pyright error rather than writing a pytest case that cannot fail). Extends to a two-sided case covered on one side, and to a predicate whose fixtures collapse the value space so a weaker predicate survives (an all-empty operand, a single-member set). This shape has been shipped **seven times** across four phases and was caught by chance every time. Provably-equivalent mutants are flagged, not given hollow tests and not deleted.
